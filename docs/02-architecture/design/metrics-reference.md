...
| How is my portfolio distributed across assets? | Position Weight — % | `asset.value / account.total_value × 100` | `stg.cmp.position_weight_pct` | The percentage of total portfolio value held in each position. High concentration in one position increases idiosyncratic risk. |
| What is my overall portfolio volatility? | Portfolio Volatility (weighted) | `Σ (weight_i × volatility_30d_i)` | `stg.acc_cmp.portfolio_volatility_weighted` | Weighted average of per-position 30D volatility. Weight = position value / total portfolio value. Approximates portfolio-level price uncertainty. |
...
| What is the current drawdown — how far is it from its peak? | Price Drawdown — % (1m / 3m / 6m / 12m) | `(price − rolling_max_price_Nd) / rolling_max_price_Nd × 100` over N ∈ {30, 90, 180, 365} trading days | `analytics.fact_technical.price_drawdown_pct_{30,90,180,365}d` | How far the asset's current price has fallen from its rolling peak over the chosen horizon. Asset-only and position-size-independent — primary input to the DCA automation script. NULL until the ticker has the full window of trading-day history. The legacy `value_drawdown_pct_30d` (position-value based) remains available for the dashboard. |
| Is now a good time to buy more (DCA opportunity)? | DCA Bias | `current_price / avg_price` | `stg.cmp.dca_bias` | A ratio below 1.0 means the current price is below your average cost — buying now would lower your average. Above 1.0 means buying now would raise it. The further below 1.0, the stronger the DCA signal. Note: formula was previously computed incorrectly as `(value - cost) / cost` (P&L %) — corrected in pipeline_asset_computed_silver.py. |
| What is the risk of buying more right now? | Volatility (30D) + Price Drawdown | Volatility: `std_dev(daily_returns, 30 days)` × `√252` to annualise; Drawdown: `price_drawdown_pct_{30,90,180,365}d` | `stg.cmp.volatility_30d`, `analytics.fact_technical.price_drawdown_pct_*` | High volatility means price is swinging widely — buying into high volatility increases the chance of an immediate paper loss. Deep drawdown signals the asset is significantly off its peak across the chosen horizon. |
...
| What percentage of my holding is locked in pies vs. available to sell? | Pie-locked Quantity Ratio | `quantity_in_pies / quantity × 100` | `stg.ast.quantity_in_pies`, `stg.ast.share` | Shows liquidity of the position. Shares held inside Trading212 pies may have restricted selling. High pie ratio means most of the holding cannot be freely sold. |
