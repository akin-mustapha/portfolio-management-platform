...
def _daily_change_card(daily_change_pct, daily_change_series, theme="light"):
    change_sign = (
        1 if (daily_change_pct or 0) > 0 else -1 if (daily_change_pct or 0) < 0 else 0
    )
    change_class = (
        "kpi-change-positive"
        if change_sign > 0
...
def _dark_kpi_card(
    label, value_str, unit="", change_str="", change_sign=0, extra_class=""
):
    change_class = (
        "kpi-change-positive"
        if change_sign > 0
        ...
        [
            ...
            html.Div(
                [
                    ...
                    (
                        html.Span(change_str, className=f"kpi-change {change_class}")
                        if change_str
                        else None
                    ),
                ],
