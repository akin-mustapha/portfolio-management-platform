...
class AccountGoldRecord(BaseModel):
    ...
    cash_deployment_ratio: Optional[float] = None  # (total_value - cash_available) / total_value

    # Cross-asset aggregates
    fx_impact_total: Optional[float] = None               # SUM(staging.asset.fx_impact)
    portfolio_volatility_weighted: Optional[float] = None  # Σ(weight_i * volatility_30d_i)
