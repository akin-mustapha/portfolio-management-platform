...
class CredentialsRepository(BaseTableRepository):
    ...
    def save(self, provider: str, api_key: str, secret_token: str = None, api_url: str = None) -> None:
        """Upsert credentials for a provider. One row per provider."""
        self.upsert(
            [{"provider": provider, "api_key": api_key, "secret_token": secret_token or "", "api_url": api_url or ""}],
            unique_key=["provider"],
        )
