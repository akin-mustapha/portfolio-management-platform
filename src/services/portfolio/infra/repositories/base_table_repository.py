...
class BaseTableRepository(RepositoryInterface):
    def __init__(self, entity_name: str, schema_name: str = None, field_map: Dict[str, str] = None):
      ...
      """
      super().__init__(entity_name=entity_name, schema_name=schema_name)
    ...
    def insert(self, records: Iterable[Dict]):
      ...
      for record in records:
        ...
        sql = f"INSERT INTO {self.schema_name}.{self.entity_name} ({columns}) VALUES ({placeholders})"
        
        logging.debug(f"Executing query: {sql} with params: {record}")
        
        with self._client as client:
          res = client.execute(sql, record)
        logging.info(f"Inserted record into {self.schema_name}.{self.entity_name}")
    ...
    def upsert(self, records: Dict, unique_key: list[str]):
      for record in records:
        ...
        sql = f"""
          INSERT INTO {self.schema_name}.{self.entity_name} ({columns})
          VALUES ({placeholders})
        ...
        with self._client as client:
          res = client.execute(sql, record)
        logging.info(f"Upserted record into {self.schema_name}.{self.entity_name}")
      return res
    ...
    def select(self, params: Dict):
      ...
      if result:
        ...
        sql = f"SELECT * FROM {self.schema_name}.{self.entity_name} WHERE {' AND '.join(filters)}"
        
        logging.info(f"Selecting record from {self.schema_name}.{self.entity_name}")
    ...
    def select_all(self):
      sql = f"SELECT * FROM {self.schema_name}.{self.entity_name}"
      
      logging.info(f"Selecting all records from {self.schema_name}.{self.entity_name}")
    ...
    def update(self, params: Dict, data: Dict):
      ...
      sql = f"UPDATE {self.schema_name}.{self.entity_name} SET {set_clause} WHERE {where_clause}"
      combined_params = {**db_data, **db_params}
      
      logging.info(f"Updating record in {self.schema_name}.{self.entity_name}")
      
      with self._client as client:
          res = client.execute(sql, combined_params)
          
      logging.info(f"Updated record in {self.schema_name}.{self.entity_name}")
      return res
    ...
    def delete(self, params: Dict):
      ...
      sql = f"DELETE FROM {self.schema_name}.{self.entity_name} WHERE {where_clause}"
      
      logging.info(f"Deleting record from {self.schema_name}.{self.entity_name}")
      
      with self._client as client:
          res = client.execute(sql, db_params)
          
      logging.info(f"Deleted record from {self.schema_name}.{self.entity_name}")
