...
class TestTagToRecord:
    ...
    def test_to_record_raises_if_tag_type_id_missing(self):
        ...
        tag = Tag(id=None, name="Orphan", description="", tag_type_id=0, created_timestamp="2026-01-01")
        record = tag.to_record()
...
class TestCategoryToRecord:
    def test_to_record_returns_expected_keys(self):
        cat = Category(id=None, name="Equity", description="Equity assets", created_timestamp="2026-01-01")
        record = cat.to_record()

        assert set(record.keys()) == {"name", "description", "created_timestamp"}

    def test_to_record_values_match(self):
        cat = Category(id=5, name="Bond", description="Fixed income", created_timestamp="2026-03-01")
        record = cat.to_record()
...
class TestIndustryToRecord:
    def test_to_record_includes_id(self):
        ind = Industry(id=10, name="Technology", description="Tech sector", created_timestamp="2026-01-01")
...
class TestSectorToRecord:
    def test_to_record_includes_industry_id(self):
        sec = Sector(id=3, industry_id=1, name="Software", description="SW companies", created_timestamp="2026-01-01")
    ...
    def test_to_record_with_null_industry_id(self):
        ...
        sec = Sector(id=None, industry_id=None, name="Orphan Sector", description="", created_timestamp=None)
        record = sec.to_record()
