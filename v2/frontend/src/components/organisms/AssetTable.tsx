...
function getRowClassName(params: {
  ...
  const sentiment = !Number.isFinite(profit) || profit === 0
    ? ''
    : profit > 0
      ? 'row-positive'
      : 'row-negative'
...
const NUM: Pick<GridColDef, 'align' | 'headerAlign'> = { align: 'right', headerAlign: 'right' }

function buildColumns(theme: Theme, onOpenDetails?: (ticker: string) => void): GridColDef[] {
  return [
    {
      field: 'price_series',
      headerName: '30D',
    ...
    { field: 'value', headerName: 'Value', width: 110, valueFormatter: (v) => numFmt(v), ...NUM },
    { field: 'cost', headerName: 'Cost', width: 110, valueFormatter: (v) => numFmt(v), ...NUM },
    {
      field: 'profit',
      headerName: 'P&L',
      width: 110,
    ...
    { field: 'weight_pct', headerName: 'Weight %', width: 90, valueFormatter: pctFmt, renderHeader: headerWithInfo('Weight %', 'weight_pct'), ...NUM },
    {
    ...
    { field: 'volatility_30d', headerName: 'Vol 30d', width: 90, valueFormatter: (v) => numFmt(v, 4), renderHeader: headerWithInfo('Vol 30d', 'volatility_30d'), ...NUM },
    { field: 'volatility_50d', headerName: 'Vol 50d', width: 90, valueFormatter: (v) => numFmt(v, 4), ...NUM },
    { field: 'var_95_1d', headerName: 'VaR 95%', width: 90, valueFormatter: (v) => numFmt(v, 4), renderHeader: headerWithInfo('VaR 95%', 'var_95_1d'), ...NUM },
    {
      field: 'beta_60d',
      headerName: 'Beta 60d',
      width: 90,
    ...
    { field: 'value_ma_30d', headerName: 'MA 30d', width: 90, valueFormatter: (v) => numFmt(v), ...NUM },
    { field: 'value_ma_50d', headerName: 'MA 50d', width: 90, valueFormatter: (v) => numFmt(v), ...NUM },
    {
    ...
    { field: 'fx_impact', headerName: 'FX Impact', width: 90, valueFormatter: (v) => numFmt(v), ...NUM },
    {
      field: 'tags',
      headerName: 'Tags',
      width: 150,
...
export default function AssetTable({ rows, loading = false, availableTags = [], onOpenDetails }: AssetTableProps) {
  const theme = useTheme()
  const { selectedTags, setSelectedTags, density } = useAppStore()

  const columns = useMemo(() => buildColumns(theme, onOpenDetails), [theme, onOpenDetails])
