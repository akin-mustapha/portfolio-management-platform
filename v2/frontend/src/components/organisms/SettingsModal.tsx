...
export default function SettingsModal({ open, onClose }: SettingsModalProps) {
  ...
  const save = useMutation({
    mutationFn: () => saveCredentials({ api_key: apiKey, secret_token: secretToken, api_url: apiUrl }),
  ...
  return (
    <Dialog open={open} onClose={onClose} maxWidth="xs" fullWidth>
      <DialogTitle sx={{ fontSize: 14, fontWeight: 700 }}>API Credentials</DialogTitle>
      <DialogContent sx={{ display: 'flex', flexDirection: 'column', gap: 2, pt: 2 }}>
        {success && <Alert severity="success" sx={{ fontSize: 12 }}>Credentials saved.</Alert>}
