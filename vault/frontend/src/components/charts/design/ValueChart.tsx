...
export default function ValueChart({
  ...
  const linePath = slice
    .map(
      (d, i) =>
        (i ? "L" : "M") + xPos(i).toFixed(1) + " " + yPos(d.v).toFixed(1),
    )
    .join(" ");
  const areaPath =
    linePath +
    ` L ${xPos(slice.length - 1)} ${padT + innerH} L ${xPos(0)} ${padT + innerH} Z`;
  ...
  const onMove = (e: React.MouseEvent) => {
    ...
    const idx = Math.max(
      ...
      Math.min(
        slice.length - 1,
        Math.round(((px - padL) / innerW) * (slice.length - 1)),
      ),
    );
      ...
      {hover && (
        <div
          style={{
            ...
            padding: "6px 10px",
            fontSize: 11.5,
            boxShadow: (theme as Theme & { custom: { shadowPop: string } })
              .custom.shadowPop,
            pointerEvents: "none",
            minWidth: 120,
        ...
        >
          ...
          {slice.length > 0 && (
            <div style={{ color: theme.palette.success.main, fontSize: 11 }}>
              {fmtPct(((hover.v - slice[0].inv) / slice[0].inv) * 100)} vs
              invested
            </div>
          )}
        </div>
      ...
      {["1W", "1M", "3M", "6M", "1Y", "ALL"].map((r) => (
        <button
          ...
          style={{
            ...
            background:
              range === r ? theme.palette.primary.main + "18" : "transparent",
            color:
              range === r
