...
type DataPoint = PortfolioValueChartRow

function CustomTooltip({ active, payload, theme }: { active?: boolean; payload?: unknown[]; theme: ReturnType<typeof useTheme> }) {
...
export default function PortfolioValueChart({ rows }: PortfolioValueChartProps) {
  const theme = useTheme()
  if (!rows?.length) return null

  const hasPnl = rows.some((r) => r.realized != null)

  return (
    <ResponsiveContainer width="100%" height={260}>
      <ComposedChart data={rows} margin={{ top: 4, right: 50, bottom: 0, left: 0 }}>
        <defs>
        ...
        <Tooltip content={(props) => <CustomTooltip active={props.active} payload={props.payload as unknown[]} theme={theme} />} />
