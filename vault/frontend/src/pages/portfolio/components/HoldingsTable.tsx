...
function makeSparkline(asset: RawAsset): number[] {
  ...
  const r = seeded(
    asset.ticker.charCodeAt(0) * 31 + (asset.ticker.charCodeAt(1) || 7),
  );
  const arr: number[] = [];
  let v = 100;
  for (let i = 0; i < 30; i++) {
...
export default function HoldingsTable({
  ...
  const hov = (theme as Theme & { custom: { bgRowHover: string } }).custom
    .bgRowHover;
  ...
  const headers: {
    ...
    { key: "value", label: "Value", fr: "minmax(80px, 1fr)", align: "left" },
    {
      key: "weight_pct",
      label: "Weight",
      fr: "minmax(120px, 1.4fr)",
      align: "left",
    },
    {
      key: "daily_value_return",
      label: "Today",
      fr: "minmax(64px, 1fr)",
      align: "right",
    },
        ...
        {headers.map((h) => (
          ...
          >
            ...
            {h.key && sort.key === h.key && (
              <span
                style={{ marginLeft: 4, color: theme.palette.primary.main }}
              >
                {sort.dir < 0 ? "↓" : "↑"}
              </span>
            )}
    ...
    >
      ...
      <div
        style={{
          ...
          fontVariantNumeric: "tabular-nums",
          fontWeight: 600,
          color: dayRet >= 0 ? upColor : dnColor,
        }}
      >
