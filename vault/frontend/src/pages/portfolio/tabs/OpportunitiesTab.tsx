...
function Rebalance({
  ...
  assets.forEach((a) => {
    const sector =
      ((a as Record<string, unknown>).sector as string) ??
      a.tags?.[0] ??
      "Other";
    current[sector] =
      (current[sector] ?? 0) + (Number(a.value) || 0) / (totalValue || 1);
  });
  const rows = Object.keys(TARGET_WEIGHTS)
    .map((s) => ({
      sector: s,
      current: current[s] ?? 0,
      target: TARGET_WEIGHTS[s],
  ...
  return (
    <CardShell
      badge="Rebalance"
      badgeColor="oklch(0.92 0.06 250 / 0.6)"
          ...
          <div key={r.sector}>
            ...
            >
              ...
              >
                ...
                >
                  {r.delta > 0 ? "trim " : "add "}
                  {fmtEUR(Math.abs(r.delta) * portfolioValue, 0)}
                </span>
              </span>
            </div>
            <div
...
function TaxLoss({ assets }: { assets: RawAsset[] }) {
  ...
  return (
    <CardShell
      badge="Tax-loss"
      badgeColor="oklch(0.92 0.07 80 / 0.6)"
...
const CASH_IDEAS = [
  {
    ...
    why: "Diversifies away from tech",
    alloc: 400,
  },
  {
    ticker: "IWDA",
    name: "iShares Core MSCI World",
    why: "Broad ex-US exposure",
    alloc: 500,
  },
  {
    ticker: "XEON",
    name: "Xtrackers Eurozone Gov Bond",
    why: "Ballast vs equity drawdown",
    alloc: 240,
  },
];
...
function CashDeploy({ cash }: { cash: number }) {
  ...
  return (
    <CardShell
      badge="Deploy cash"
      badgeColor="oklch(0.92 0.06 152 / 0.6)"
...
const DIVIDENDS = [
  ...
  { ticker: "NKE", date: "Jun  2", perShare: 0.4, qty: 8 },
];
    ...
    >
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8 }}>
        {DIVIDENDS.map((e) => {
          ...
          const sector = asset
            ? (((asset as Record<string, unknown>).sector as string) ??
              asset.tags?.[0] ??
              "")
            : "Tech";
          return (
            ...
            >
              ...
              <div style={{ minWidth: 0 }}>
                ...
                >
                  {e.date} · €{e.perShare.toFixed(2)}/sh
                </div>
              </div>
              <span
                style={{
...
function Earnings({ assets }: { assets: RawAsset[] }) {
  ...
  return (
    <CardShell
      badge="Earnings"
      badgeColor="oklch(0.92 0.06 60 / 0.6)"
...
function ConcentrationWarn({
  ...
  return (
    <CardShell
      badge="Risk alert"
      badgeColor="oklch(0.92 0.08 25 / 0.6)"
            ...
            >
              ...
              <div style={{ flex: 1, minWidth: 0 }}>
                ...
                >
                  ...
                  weight · suggest trim to 5% (≈{" "}
                  {fmtEUR((a.w - 0.05) * portfolioValue, 0)})
                </div>
              </div>
...
function SectorGaps({ assets }: { assets: RawAsset[] }) {
  ...
  const rows = Object.keys(BENCHMARK).map((s) => ({
    ...
    you: current[s] ?? 0,
    bm: BENCHMARK[s],
    gap: (current[s] ?? 0) - BENCHMARK[s],
  }));

  return (
    <CardShell
      badge="Gaps"
      badgeColor="oklch(0.92 0.05 200 / 0.6)"
            ...
            >
              ...
              <div
                style={{
                  ...
                  height: "100%",
                  left: 0,
                  width: Math.min(r.you * 200, 100) + "%",
                  background:
                    r.gap < 0 ? "oklch(0.78 0.12 200)" : "oklch(0.78 0.13 30)",
                  opacity: 0.7,
                  borderRadius: 3,
                }}
              />
...
const SIMILAR_PICKS = [
  {
    ...
    because: "Semi peer of NVDA / ASML",
    perf: 18.4,
  },
  {
    ticker: "AMAT",
    name: "Applied Materials",
    because: "Semi peer to ASML",
    perf: 12.1,
  },
  {
    ticker: "CRWD",
    name: "CrowdStrike",
    because: "Cyber (complements ETFs)",
    perf: 24.0,
  },
  {
    ticker: "NOW",
...
function SimilarPicks() {
  ...
  return (
    <CardShell
      badge="Discover"
      badgeColor="oklch(0.92 0.05 280 / 0.6)"
