...
export function presentAssetKpis(
...
export function presentAssetHistory(
  raw: RawAssetHistoryResponse,
): AssetHistoryVM {
  ...
  return {
    ...
    asset_return: {
      ...
      values: (rawReturn?.values ?? []).map((v) =>
        v != null ? Number(v) * 100 : null,
      ),
    },
  };
}
