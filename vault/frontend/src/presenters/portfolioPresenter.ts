...
function dailyChangeSeries(history: RawPortfolioHistoryRow[]) {
  const rows = history
    .filter((r) => r.daily_value_change_pct != null)
    .slice(-30);
  return {
...
function kpi(snapshot: RawSnapshot, history: RawPortfolioHistoryRow[]): KpiVM {
  ...
  return {
    ...
    daily_value_change_pct:
      snapshot.daily_value_change_pct != null
    ...
    portfolio_beta:
      snapshot.portfolio_beta_weighted != null
        ...
        : null,
    sharpe_ratio_30d:
      snapshot.sharpe_ratio_30d != null
...
export function portfolioValueSeries(
  rows: RawPortfolioHistoryRow[],
): PortfolioValueSeriesVM {
  return {
    dates: rows.map((r) => String(r.data_date)),
    values: rows.map(
      (r) =>
        toFloat(r.investments_total_cost) +
        toFloat(r.investments_unrealized_pnl),
    ),
...
export function portfolioPnlSeries(
  ...
  return {
    ...
    total_pnl: rows.map(
      (r) =>
        toFloat(r.investments_unrealized_pnl) +
        toFloat(r.investments_realized_pnl),
    ),
  };
}
...
function positionWeightSeries(assets: RawAsset[]): PositionWeightSeriesVM {
  ...
  if (restTotal > 0) {
    const breakdown = rest
      .map((i) => `${i.ticker}: ${round(i.weight_pct, 1)}%`)
      .join("\n");
...
function topByField(
  ...
  return [...assets]
    ...
    .sort((a, b) =>
      direction === "desc" ? b[sortBy] - a[sortBy] : a[sortBy] - b[sortBy],
    )
    .slice(0, 10);
}
...
function dailyMovers(assets: RawAsset[]): DailyMoverItem[] {
  ...
  return items
    .sort(
      (a, b) => Math.abs(b.daily_value_return) - Math.abs(a.daily_value_return),
    )
    .slice(0, 15);
}
...
function profitability(
  assets: RawAsset[],
): Array<{ ticker: string; profit: number }> {
  return assets.map((a) => ({ ticker: a.ticker, profit: toFloat(a.profit) }));
}

function losersPnl(
  history: RawAssetHistoryRow[],
...
function portfolioPnlChartRows(
  pnlSeries: PortfolioPnlSeriesVM,
): PortfolioPnlChartRow[] {
...
export function presentPortfolioSummary(
