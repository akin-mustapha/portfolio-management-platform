"""015_staging_drawdown_windows

Adds rolling price-high columns at 30/90/180/365 day windows to
`staging.v_asset_metrics` so the gold layer can derive price-based drawdown
over multiple horizons (used by DCA automation).

Each rolling MAX is gated on having a full window's worth of rows
(`COUNT(price) OVER w_N = N`); otherwise the value is NULL. This avoids
short-history tickers reporting an artificially shallow drawdown simply
because their window only contains a few days.

`v_account_metrics` depends on `v_asset_metrics` and is dropped by CASCADE
when the latter is recreated, so this migration recreates both views. The
`v_account_metrics` definition is unchanged from migration 014.

Revision ID: 2200000000b15
Revises: 2200000000b14
...
revision: str = "2200000000b15"
down_revision: Union[str, Sequence[str], None] = "2200000000b14"
...
_V_ASSET_METRICS_V2 = """
    ...
    SELECT
        ...
        CASE WHEN COUNT(price) OVER w_30  = 30  THEN MAX(price) OVER w_30  END
            AS recent_price_high_30d,
        CASE WHEN COUNT(price) OVER w_90  = 90  THEN MAX(price) OVER w_90  END
            AS recent_price_high_90d,
        CASE WHEN COUNT(price) OVER w_180 = 180 THEN MAX(price) OVER w_180 END
            AS recent_price_high_180d,
        CASE WHEN COUNT(price) OVER w_365 = 365 THEN MAX(price) OVER w_365 END
            AS recent_price_high_365d,
...
def upgrade() -> None:
    ...
    op.execute(_V_ASSET_METRICS_V2)
...
def downgrade() -> None:
    op.execute("DROP MATERIALIZED VIEW IF EXISTS staging.v_account_metrics CASCADE")
    op.execute("DROP MATERIALIZED VIEW IF EXISTS staging.v_asset_metrics CASCADE")
    op.execute(_V_ASSET_METRICS_V1)
    op.execute(_V_ACCOUNT_METRICS)
